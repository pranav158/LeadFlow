...
(function () {
  ...
  function bindEvents() {
    ...
    // Search
    searchInput.addEventListener('input', debounce(() => filterLeads(), 250));
    ...
    logsPagination.addEventListener('click', handlePaginationClick);
  ...
  async function loadLeads() {
    showLoading(true);
    try {
      const res = await fetch(`/api/leads?status=${currentTab}`);
  ...
  function filterLeads() {
    ...
    const filtered = allLeads.filter(l =>
      l.companyName.toLowerCase().includes(q) || l.email.toLowerCase().includes(q)
    );
